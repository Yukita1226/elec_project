...
// ─────────────────────────────────────────────────────────────────────────────
//  PALETTE  (tweak hex values to match your brand)
// ─────────────────────────────────────────────────────────────────────────────
#define CLR_BG          lv_color_hex(0x0D1117)   // near-black background
#define CLR_CARD        lv_color_hex(0x161B22)   // card surface
#define CLR_CARD_SOLAR  lv_color_hex(0xFFB300)   // amber  – solar
#define CLR_CARD_EV     lv_color_hex(0x00BCD4)   // cyan   – EV station
#define CLR_CARD_CAR    lv_color_hex(0x66BB6A)   // green  – electric car
#define CLR_TEXT        lv_color_hex(0xE6EDF3)   // primary text
#define CLR_SUBTEXT     lv_color_hex(0x8B949E)   // secondary / label text
#define CLR_DIVIDER     lv_color_hex(0x21262D)   // subtle border
#define CLR_POSITIVE    lv_color_hex(0x3FB950)   // "export" indicator
#define CLR_NEGATIVE    lv_color_hex(0xF85149)   // "import" indicator
 
// ─────────────────────────────────────────────────────────────────────────────
//  CHART HISTORY  (circular buffer, one point per refresh)
// ─────────────────────────────────────────────────────────────────────────────
#define CHART_POINTS    20      // how many samples shown in the graph
 
static lv_chart_series_t * ser_solar = nullptr;
static lv_chart_series_t * ser_ev    = nullptr;
static lv_chart_series_t * ser_car   = nullptr;
static lv_obj_t           * chart    = nullptr;
 
// ─────────────────────────────────────────────────────────────────────────────
//  LIVE WIDGETS (updated on every refresh)
// ─────────────────────────────────────────────────────────────────────────────
struct CardWidgets {
    lv_obj_t * val_ptotal;      // big kW number
    lv_obj_t * val_ptransfer;
    lv_obj_t * flow_dot;        // green = export, red = import
    lv_obj_t * bar;             // power bar
};
 
static CardWidgets w_solar, w_ev, w_car;
...
//  HELPERS
// ─────────────────────────────────────────────────────────────────────────────
static void set_card_value(const CardWidgets & w,
                           int32_t ptotal, int32_t ptransfer,
                           lv_color_t accent)
{
    char buf[64];
 
    // Main power value
    snprintf(buf, sizeof(buf), "%.2f kW", ptotal / 1000.0f);
    lv_label_set_text(w.val_ptotal, buf);
 
    // Transfer
    snprintf(buf, sizeof(buf), ptransfer >= 0 ? "+%.0f W" : "%.0f W", (float)ptransfer);
    lv_label_set_text(w.val_ptransfer, buf);
    lv_obj_set_style_text_color(w.val_ptransfer,
        ptransfer >= 0 ? CLR_POSITIVE : CLR_NEGATIVE, LV_PART_MAIN);
 
    // Flow dot
    lv_obj_set_style_bg_color(w.flow_dot,
        ptransfer >= 0 ? CLR_POSITIVE : CLR_NEGATIVE, LV_PART_MAIN);
 
    // Bar  (cap at 10 000 W for full scale)
    int32_t bar_val = (ptotal * 100) / 10000;
...
// ─────────────────────────────────────────────────────────────────────────────
//  DATA FETCH & UI UPDATE  (called by timer + button)
// ─────────────────────────────────────────────────────────────────────────────
static void do_refresh(lv_timer_t * /*t*/ = nullptr)
{
    ...
    set_card_value(w_ev,    ev_data.Ptotal,    ev_data.Ptransfer,    CLR_CARD_EV);
    set_card_value(w_car,   car_data.Ptotal,   car_data.Ptransfer,   CLR_CARD_CAR);
 
    // Push new chart points  (LVGL 8: lv_chart_set_next_value)
    lv_chart_set_next_value(chart, ser_solar, (lv_coord_t)(solar_data.Ptotal / 10));
    lv_chart_set_next_value(chart, ser_ev,    (lv_coord_t)(ev_data.Ptotal   / 10));
    lv_chart_set_next_value(chart, ser_car,   (lv_coord_t)(car_data.Ptotal  / 10));
    lv_chart_refresh(chart);
 
    // Timestamp
    // If your platform has time(), use it; otherwise a counter is fine
...
static CardWidgets build_card(lv_obj_t * parent,
                              const char * title,
                              const char * icon,      // UTF-8 symbol or LV_SYMBOL_*
                              lv_color_t accent,
                              ...
                              lv_coord_t w, lv_coord_t h)
{
    ...
    lv_obj_set_style_border_opa(card,   120,         LV_PART_MAIN);  // subtle
                        ...
                        lv_coord_t w, lv_coord_t h)
{
    ...
    lv_label_set_text(ttl, "Power History (W × 10)");
    ...
    const struct { const char * name; lv_color_t color; lv_coord_t ox; } legend[] = {
        { "Solar",    CLR_CARD_SOLAR, 0   },
        { "EV",       CLR_CARD_EV,    70  },
        { "Car",      CLR_CARD_CAR,   130 },
    };
    for (auto & l : legend) {
        lv_obj_t * d = lv_obj_create(cont);
        lv_obj_set_size(d, 8, 8);
        lv_obj_set_pos(d, w - 165 + l.ox, 2);
        ...
        lv_obj_set_pos(ld, w - 153 + l.ox, 0);
    ...
    lv_chart_set_range(chart, LV_CHART_AXIS_PRIMARY_Y, 0, 1000);   // 0–10 000 W
 
    lv_obj_set_style_bg_color(chart,      CLR_CARD,    LV_PART_MAIN);
    lv_obj_set_style_border_width(chart,  0,           LV_PART_MAIN);
    lv_obj_set_style_line_color(chart,    CLR_DIVIDER, LV_PART_MAIN);  // grid
    lv_obj_set_style_line_width(chart,    1,           LV_PART_MAIN);
    lv_chart_set_div_line_count(chart, 4, 5);
 
    // Hide the dot markers for a clean line look
    lv_obj_set_style_width(chart,  0, LV_PART_INDICATOR);
    lv_obj_set_style_height(chart, 0, LV_PART_INDICATOR);
 
    // Add series
    ser_solar = lv_chart_add_series(chart, CLR_CARD_SOLAR, LV_CHART_AXIS_PRIMARY_Y);
    ser_ev    = lv_chart_add_series(chart, CLR_CARD_EV,    LV_CHART_AXIS_PRIMARY_Y);
    ser_car   = lv_chart_add_series(chart, CLR_CARD_CAR,   LV_CHART_AXIS_PRIMARY_Y);
 
    // Pre-fill with zeros so the graph looks clean on startup
    for (int i = 0; i < CHART_POINTS; i++) {
        lv_chart_set_next_value(chart, ser_solar, 0);
        lv_chart_set_next_value(chart, ser_ev,    0);
...
void create_page_t2()
{
    ...
    lv_label_set_text(title, LV_SYMBOL_CHARGE "  Energy Flow Monitor");
    ...
    // ── Card row ────────────────────────────────────────────────────────────
    //  Lay three equal cards side-by-side below the header
    //  Adjust x/y/w/h to fit your screen resolution.
    //  Values below assume 480 × 320 display.
    const lv_coord_t CARD_Y   = 48;
    const lv_coord_t CARD_H   = 148;
    const lv_coord_t CARD_W   = 142;
    const lv_coord_t CARD_GAP = 6;
 
    w_solar = build_card(scr, "SOLAR PANEL",    LV_SYMBOL_IMAGE,    CLR_CARD_SOLAR,
                          CARD_GAP,                        CARD_Y, CARD_W, CARD_H);
    w_ev    = build_card(scr, "EV STATION",     LV_SYMBOL_CHARGE,   CLR_CARD_EV,
                          CARD_GAP + CARD_W + CARD_GAP,   CARD_Y, CARD_W, CARD_H);
    w_car   = build_card(scr, "ELECTRIC CAR",   LV_SYMBOL_DRIVE,    CLR_CARD_CAR,
